...
class data extends Controller
{
    ...
    public function remove_saved_template(Request $request)
    {
        if (trim($request->action) == "excluir") {
            $result = DB::table('temp_parceiros')
                ->where('temp_parceiro_id', '=', $request->id)->where('dominio', '=', $request->dominio)
                ->delete();
    ...
    public function save_template(Request $request)
    {
        ...
        if ($count->total == 0) {
            ...
            //$this->close_current_window($response);
            return response()->json($response);
    ...
    private function close_current_window($msg)
    {
        echo '<script>';
        echo 'alert("' . $msg . '");'; // Mostra a mensagem
