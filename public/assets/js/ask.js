...
ask_header.forEach(ask => {
    ask.addEventListener('click', function (e) {
        ask_header.forEach(asks => {asks.nextElementSibling.classList.remove('active');});
        var icon = e.target.querySelector('.ask-icon');
        if (icon.className.includes('fa-plus')) { icon.classList.remove('fa-plus'); icon.classList.add('fa-minus'); }
        else { icon.classList.remove('fa-minus'); icon.classList.add('fa-plus'); }
        var container = e.target.parentNode;
        var next = e.target.nextElementSibling;
        const initialHeight = next.offsetHeight;
        animateAuto(next);
    });
